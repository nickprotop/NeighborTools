...
echo "========================"
echo "Configure passwords for the PostgreSQL database setup."
...
POSTGRES_PASSWORD=$(read_password "PostgreSQL password" "ToolsPassword123!")
...
COMPOSE_PROJECT_NAME=toolssharing
...
echo "📦 Setting up infrastructure (PostgreSQL, Redis)..."
...
CONNECTION_STRING="Host=localhost;Port=5433;Database=toolssharing;Username=toolsuser;Password=${POSTGRES_PASSWORD}"
if [ "$ENABLE_REDIS_PASSWORD" = "true" ]; then
...
echo "  • PostgreSQL: localhost:5433 (user: toolsuser, password: [configured above])"
if [ "$ENABLE_REDIS_PASSWORD" = "true" ]; then
