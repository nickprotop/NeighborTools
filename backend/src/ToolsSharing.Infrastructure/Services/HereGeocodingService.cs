using Microsoft.EntityFrameworkCore;
using Microsoft.Extensions.Caching.Memory;
using Microsoft.Extensions.Logging;
using System.Text;
using System.Text.Json;
using ToolsSharing.Core.Configuration;
using ToolsSharing.Core.DTOs.Location;
using ToolsSharing.Core.Interfaces;
using ToolsSharing.Infrastructure.Data;

namespace ToolsSharing.Infrastructure.Services;

/// <summary>
/// HERE Maps geocoding service implementation
/// </summary>
public class HereGeocodingService : IGeocodingService
{
    private readonly HttpClient _httpClient;
    private readonly ApplicationDbContext _context;
    private readonly IMemoryCache _cache;
    private readonly ILogger<HereGeocodingService> _logger;
    private readonly HereConfiguration _config;
    private readonly SemaphoreSlim _rateLimiter;
    private DateTime _lastRequestTime = DateTime.MinValue;

    public string ProviderName => "HERE";

    public HereGeocodingService(
        HttpClient httpClient,
        ApplicationDbContext context,
        IMemoryCache cache,
        ILogger<HereGeocodingService> logger,
        HereConfiguration config)
    {
        _httpClient = httpClient;
        _context = context;
        _cache = cache;
        _logger = logger;
        _config = config;
        _rateLimiter = new SemaphoreSlim(1, 1);

        // Configure HttpClient
        _httpClient.BaseAddress = new Uri(_config.BaseUrl);
        _httpClient.Timeout = TimeSpan.FromSeconds(_config.TimeoutSeconds);
    }

    public async Task<List<LocationOption>> SearchLocationsAsync(string query, int limit = 5, string? countryCode = null)
    {
        if (string.IsNullOrWhiteSpace(query))
            return new List<LocationOption>();

        if (string.IsNullOrWhiteSpace(_config.ApiKey))
        {
            _logger.LogWarning("HERE API key not configured, skipping HERE geocoding");
            return new List<LocationOption>();
        }

        var cacheKey = $"here_search:{query}:{limit}:{countryCode}";
        
        // Check cache first
        if (_cache.TryGetValue(cacheKey, out List<LocationOption>? cachedResults))
        {
            _logger.LogDebug("Returning cached HERE results for query: {Query}", query);
            return cachedResults!;
        }

        try
        {
            await EnforceRateLimit();

            var queryParams = new StringBuilder();
            queryParams.Append($"q={Uri.EscapeDataString(query)}");
            queryParams.Append($"&apiKey={_config.ApiKey}");
            queryParams.Append($"&limit={Math.Min(limit, _config.MaxResults)}");
            queryParams.Append($"&lang={_config.DefaultLanguage}");

            if (_config.IncludeAddressDetails)
                queryParams.Append("&show=details");

            if (_config.IncludeMapView)
                queryParams.Append("&show=areas");

            if (!string.IsNullOrEmpty(countryCode))
                queryParams.Append($"&in=countryCode:{countryCode.ToUpperInvariant()}");

            var requestUrl = $"/geocode?{queryParams}";
            _logger.LogDebug("Making HERE request: {Url}", requestUrl.Replace(_config.ApiKey, "***"));

            var response = await _httpClient.GetAsync(requestUrl);
            
            if (!response.IsSuccessStatusCode)
            {
                _logger.LogWarning("HERE API error: {StatusCode} - {ReasonPhrase}", 
                    response.StatusCode, response.ReasonPhrase);
                return new List<LocationOption>();
            }

            var jsonContent = await response.Content.ReadAsStringAsync();
            var hereResponse = JsonSerializer.Deserialize<HereGeocodingResponse>(jsonContent);

            if (hereResponse?.Items == null || !hereResponse.Items.Any())
            {
                _logger.LogDebug("No results found for query: {Query}", query);
                return new List<LocationOption>();
            }

            // Convert to LocationOptions and apply disambiguation
            var locationOptions = hereResponse.Items
                .Select(item => item.ToLocationOption())
                .Where(option => !string.IsNullOrEmpty(option.DisplayName))
                .ToList();

            // Apply location disambiguation
            locationOptions = await ApplyLocationDisambiguationAsync(locationOptions, null);

            // Cache the results
            var cacheExpiry = TimeSpan.FromHours(_config.CacheDurationHours);
            _cache.Set(cacheKey, locationOptions, cacheExpiry);

            _logger.LogDebug("Found {Count} locations for query: {Query}", locationOptions.Count, query);
            return locationOptions;
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "HTTP error calling HERE API for query: {Query}", query);
            return new List<LocationOption>();
        }
        catch (JsonException ex)
        {
            _logger.LogError(ex, "JSON parsing error from HERE API for query: {Query}", query);
            return new List<LocationOption>();
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Unexpected error in HERE search for query: {Query}", query);
            return new List<LocationOption>();
        }
    }

    public async Task<LocationOption?> ReverseGeocodeAsync(decimal lat, decimal lng)
    {
        if (string.IsNullOrWhiteSpace(_config.ApiKey))
        {
            _logger.LogWarning("HERE API key not configured, skipping HERE reverse geocoding");
            return null;
        }

        var cacheKey = $"here_reverse:{lat:F6}:{lng:F6}";
        
        // Check cache first
        if (_cache.TryGetValue(cacheKey, out LocationOption? cachedResult))
        {
            _logger.LogDebug("Returning cached HERE reverse geocoding result for: {Lat}, {Lng}", lat, lng);
            return cachedResult;
        }

        try
        {
            await EnforceRateLimit();

            var queryParams = new StringBuilder();
            queryParams.Append($"at={lat},{lng}");
            queryParams.Append($"&apiKey={_config.ApiKey}");
            queryParams.Append($"&lang={_config.DefaultLanguage}");

            if (_config.IncludeAddressDetails)
                queryParams.Append("&show=details");

            var requestUrl = $"/revgeocode?{queryParams}";
            _logger.LogDebug("Making HERE reverse request: {Url}", requestUrl.Replace(_config.ApiKey, "***"));

            var response = await _httpClient.GetAsync(requestUrl);
            
            if (!response.IsSuccessStatusCode)
            {
                _logger.LogWarning("HERE reverse API error: {StatusCode} - {ReasonPhrase}", 
                    response.StatusCode, response.ReasonPhrase);
                return null;
            }

            var jsonContent = await response.Content.ReadAsStringAsync();
            var hereResponse = JsonSerializer.Deserialize<HereGeocodingResponse>(jsonContent);

            if (hereResponse?.Items == null || !hereResponse.Items.Any())
            {
                _logger.LogDebug("No reverse geocoding result found for: {Lat}, {Lng}", lat, lng);
                return null;
            }

            var locationOption = hereResponse.Items.First().ToLocationOption();

            // Cache the result
            var cacheExpiry = TimeSpan.FromHours(_config.CacheDurationHours);
            _cache.Set(cacheKey, locationOption, cacheExpiry);

            _logger.LogDebug("HERE reverse geocoded {Lat},{Lng} to: {DisplayName}", lat, lng, locationOption.DisplayName);
            return locationOption;
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "HTTP error in HERE reverse geocoding for: {Lat}, {Lng}", lat, lng);
            return null;
        }
        catch (JsonException ex)
        {
            _logger.LogError(ex, "JSON parsing error in HERE reverse geocoding for: {Lat}, {Lng}", lat, lng);
            return null;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Unexpected error in HERE reverse geocoding for: {Lat}, {Lng}", lat, lng);
            return null;
        }
    }

    public async Task<List<LocationOption>> GetPopularLocationsAsync(int limit = 10)
    {
        var cacheKey = $"here_popular:{limit}";
        
        // Check cache first
        if (_cache.TryGetValue(cacheKey, out List<LocationOption>? cachedResults))
        {
            return cachedResults!;
        }

        try
        {
            // Get popular locations from database based on usage frequency
            var popularLocations = await _context.LocationSearchLogs
                .Where(log => !string.IsNullOrEmpty(log.SearchQuery))
                .GroupBy(log => log.SearchQuery!.ToLower())
                .Select(group => new { Query = group.Key, Count = group.Count() })
                .OrderByDescending(item => item.Count)
                .Take(limit * 2) // Get more to filter out invalid ones
                .ToListAsync();

            var locationOptions = new List<LocationOption>();

            foreach (var popular in popularLocations)
            {
                if (locationOptions.Count >= limit)
                    break;

                // Try to geocode the popular search term
                var results = await SearchLocationsAsync(popular.Query, 1);
                if (results.Any())
                {
                    var location = results.First();
                    location.Confidence = Math.Min(1.0m, location.Confidence + (popular.Count * 0.01m)); // Boost confidence
                    locationOptions.Add(location);
                }
            }

            // Cache for a longer time since popular locations change slowly
            var cacheExpiry = TimeSpan.FromHours(_config.CacheDurationHours * 7); // 1 week
            _cache.Set(cacheKey, locationOptions, cacheExpiry);

            _logger.LogDebug("Retrieved {Count} popular locations via HERE", locationOptions.Count);
            return locationOptions;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error retrieving popular locations via HERE");
            return new List<LocationOption>();
        }
    }

    public async Task<List<LocationOption>> GetLocationSuggestionsAsync(string query, int limit = 5)
    {
        if (string.IsNullOrWhiteSpace(query) || query.Length < 2)
            return new List<LocationOption>();

        try
        {
            // For HERE, we can use autosuggest endpoint for better suggestions
            return await GetAutosuggestionsAsync(query, limit);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error getting HERE location suggestions for query: {Query}", query);
            return new List<LocationOption>();
        }
    }

    private async Task<List<LocationOption>> GetAutosuggestionsAsync(string query, int limit)
    {
        if (string.IsNullOrWhiteSpace(_config.ApiKey))
            return new List<LocationOption>();

        var cacheKey = $"here_autosuggest:{query}:{limit}";
        
        // Check cache first
        if (_cache.TryGetValue(cacheKey, out List<LocationOption>? cachedResults))
        {
            return cachedResults!;
        }

        try
        {
            await EnforceRateLimit();

            var queryParams = new StringBuilder();
            queryParams.Append($"q={Uri.EscapeDataString(query)}");
            queryParams.Append($"&apiKey={_config.ApiKey}");
            queryParams.Append($"&limit={Math.Min(limit, _config.MaxResults)}");
            queryParams.Append($"&lang={_config.DefaultLanguage}");

            var requestUrl = $"/autosuggest?{queryParams}";
            _logger.LogDebug("Making HERE autosuggest request: {Url}", requestUrl.Replace(_config.ApiKey, "***"));

            var response = await _httpClient.GetAsync(requestUrl);
            
            if (!response.IsSuccessStatusCode)
            {
                _logger.LogWarning("HERE autosuggest API error: {StatusCode} - {ReasonPhrase}", 
                    response.StatusCode, response.ReasonPhrase);
                return new List<LocationOption>();
            }

            var jsonContent = await response.Content.ReadAsStringAsync();
            var hereResponse = JsonSerializer.Deserialize<HereAutosuggestResponse>(jsonContent);

            if (hereResponse?.Items == null || !hereResponse.Items.Any())
            {
                _logger.LogDebug("No autosuggest results found for query: {Query}", query);
                return new List<LocationOption>();
            }

            var locationOptions = hereResponse.Items
                .Where(item => item.ResultType == "place")
                .Select(item => new LocationOption
                {
                    DisplayName = item.Title ?? "",
                    City = item.Address?.City,
                    State = item.Address?.State,
                    Country = item.Address?.CountryName,
                    Lat = item.Position?.Lat,
                    Lng = item.Position?.Lng,
                    Source = Core.Enums.LocationSource.HERE,
                    Confidence = item.Scoring?.QueryScore ?? 0.8m
                })
                .Where(option => !string.IsNullOrEmpty(option.DisplayName))
                .Take(limit)
                .ToList();

            // Cache the results for shorter duration since these are suggestions
            var cacheExpiry = TimeSpan.FromMinutes(30);
            _cache.Set(cacheKey, locationOptions, cacheExpiry);

            _logger.LogDebug("Generated {Count} HERE autosuggestions for query: {Query}", locationOptions.Count, query);
            return locationOptions;
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "HTTP error calling HERE autosuggest API for query: {Query}", query);
            return new List<LocationOption>();
        }
        catch (JsonException ex)
        {
            _logger.LogError(ex, "JSON parsing error from HERE autosuggest API for query: {Query}", query);
            return new List<LocationOption>();
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Unexpected error in HERE autosuggest for query: {Query}", query);
            return new List<LocationOption>();
        }
    }

    private async Task<List<LocationOption>> ApplyLocationDisambiguationAsync(List<LocationOption> locations, string? userLocation)
    {
        if (locations.Count <= 1)
            return locations;

        try
        {
            // Group locations by similar names to identify ambiguous cases
            var ambiguousGroups = locations
                .GroupBy(loc => GetCityName(loc))
                .Where(group => group.Count() > 1)
                .ToList();

            foreach (var group in ambiguousGroups)
            {
                var groupLocations = group.ToList();
                
                // Enhance display names to disambiguate
                foreach (var location in groupLocations)
                {
                    var parts = new List<string>();
                    
                    if (!string.IsNullOrEmpty(location.City))
                        parts.Add(location.City);
                    
                    if (!string.IsNullOrEmpty(location.State))
                        parts.Add(location.State);
                    
                    if (!string.IsNullOrEmpty(location.Country))
                        parts.Add(location.Country);

                    location.DisplayName = string.Join(", ", parts);
                }

                // If user location is provided, boost nearby results
                if (!string.IsNullOrEmpty(userLocation))
                {
                    foreach (var location in groupLocations)
                    {
                        if (location.State?.ToLowerInvariant().Contains(userLocation.ToLowerInvariant()) == true ||
                            location.Country?.ToLowerInvariant().Contains(userLocation.ToLowerInvariant()) == true)
                        {
                            location.Confidence += 0.2m; // Boost local results
                        }
                    }
                }
            }

            // Re-sort by confidence after disambiguation
            return locations.OrderByDescending(loc => loc.Confidence).ToList();
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error applying location disambiguation");
            return locations; // Return original list if disambiguation fails
        }
    }

    private static string GetCityName(LocationOption location)
    {
        return (location.City ?? location.Area ?? location.DisplayName.Split(',')[0]).ToLowerInvariant();
    }

    private async Task EnforceRateLimit()
    {
        await _rateLimiter.WaitAsync();
        
        try
        {
            var timeSinceLastRequest = DateTime.UtcNow - _lastRequestTime;
            var minInterval = TimeSpan.FromSeconds(1.0 / _config.RequestsPerSecond);
            
            if (timeSinceLastRequest < minInterval)
            {
                var delay = minInterval - timeSinceLastRequest;
                _logger.LogDebug("HERE rate limiting: waiting {DelayMs}ms", delay.TotalMilliseconds);
                await Task.Delay(delay);
            }
            
            _lastRequestTime = DateTime.UtcNow;
        }
        finally
        {
            _rateLimiter.Release();
        }
    }

    public void Dispose()
    {
        _rateLimiter?.Dispose();
        _httpClient?.Dispose();
    }
}

/// <summary>
/// HERE autosuggest response structure
/// </summary>
public class HereAutosuggestResponse
{
    public List<HereAutosuggestItem> Items { get; set; } = new();
}

/// <summary>
/// HERE autosuggest item
/// </summary>
public class HereAutosuggestItem
{
    public string? Title { get; set; }
    public string? ResultType { get; set; }
    public HerePosition? Position { get; set; }
    public HereAddress? Address { get; set; }
    public HereScoring? Scoring { get; set; }
}